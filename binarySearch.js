let iterativeFunction = function (arr, logToSearch) {

	let start=0, end=arr.length-1;
		
	// Enquanto start for menor do que end, a busca é da esquerda para a direita.
	while (start<=end){

		// Encontra o valor do meio
		let mid=Math.floor((start + end)/2);

		// Faz a comparação, se for igual ja retorna True.
		if (arr[mid].log === +logToSearch) return console.log('Log encontrado!', logToSearch);

		// Senão, faz a comparação do valor do meio com o nosso X, se for maior adiciona 1 ao index e reinicia o processo de busca, ou diminui 1 e e faz a mesma coisa.
...
const rl = readline.createInterface({
  ...
  rl.question("Qual o log que deseja procurar? (Numero): ", async function (answer) {
	...
	iterativeFunction(arr, answer, 0, arr.length-1);
