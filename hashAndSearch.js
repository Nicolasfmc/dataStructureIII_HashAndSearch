...
function binarySearchFunc(arr, logToSearch) {
  ...
  // Exemplo de uso: buscar objetos por mês e log
  const results1 = searchByMonthAndLog('November', 1289);
  console.log(results1);
  
  // Exemplo de uso: buscar um log em todos os meses
  const results2 = searchByLog(4321);
...
class HashTable {
	...
	// Função para procurar objetos por mês e log
  searchByMonthAndLog(month, log) {
		const monthLogs = hashTable.get(month);
		const result = [];
		if (monthLogs) {
	...
	// Função para buscar um log em todos os meses
	searchByLog(log) {
		const result = [];
		for (const key in hashTable.table) {
			const monthLogs = hashTable.table[key].table;
			if (monthLogs[log]) {
  ...
  // Criação da hash table para separar os objetos por mês e log
...
// Método de solicitação de input do usuário.
...
const rl = readline.createInterface({
  ...
  rl.question("Qual o log que deseja procurar? (Numero): ", async function (answer) {
	const start = Date.now();
	
	binarySearchFunc(arr, answer, 0, arr.length-1);
